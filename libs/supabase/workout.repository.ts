...
export const WorkoutRepository = {
  ...
  async logSet(rawPayload: ValidatedSetInsert): Promise<WorkoutSet> {
    ...
    if (!existingSession) {
      const sessionRecord: WorkoutSession = {
        ...
        program_id: null,
        program_day_id: null,
        name: (rawPayload as any).session_name || (rawPayload as any).workout_name || (rawPayload as any).name || 'Push A (Hypertrophy)',
        status: 'in_progress',
        started_at: now,
        completed_at: null,
        duration_minutes: 0,
  ...
  async createSession(rawPayload: ValidatedSessionInsert): Promise<WorkoutSession> {
    ...
    const session: WorkoutSession = {
      ...
      name: validated.name || (rawPayload as any).workout_name || 'Push A (Hypertrophy)',
